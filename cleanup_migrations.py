#!/usr/bin/env python3
"""
Comprehensive Migration Data Cleanup Script
Fixes data quality issues in SQL migration files for PostgreSQL compatibility
"""
...
def clean_migration_file(filename):
    ...
    try:
        ...
        # 2. Fix empty strings in VALUES clauses
        lines = content.split('\n')
        result = []
        in_values = False
        ...
        # 3. Fix NULL names (customers table)
        # Pattern: INSERT with NULL as second field (name field)
        if 'customers' in filename:
            # Find patterns like: (id, NULL, and replace with (id, 'Unknown Customer',
            pattern3 = r"(\(\s*\d+\s*,\s*)NULL(\s*,)"
            matches3 = len(re.findall(pattern3, content))
            if matches3 > 0:
                content = re.sub(pattern3, r"\1'Unknown Customer'\2", content)
                changes.append(f"  ✓ Fixed {matches3} NULL customer names")
        
        # 4. Fix NULL product names (products table)
        if 'products' in filename:
            pattern4 = r"(\(\s*\d+\s*,\s*)NULL(\s*,)"
            matches4 = len(re.findall(pattern4, content))
            if matches4 > 0:
                content = re.sub(pattern4, r"\1'Unknown Product'\2", content)
                changes.append(f"  ✓ Fixed {matches4} NULL product names")
        
        # 5. Ensure proper NULL handling for optional integer fields
        # Replace patterns like: , '', <number> with , NULL, <number>
        content = re.sub(r",\s*''\s*,\s*(\d+)", r', NULL, \1', content)
        
        # 6. Fix invalid coordinate placeholders (X, Y, x, y)
        content = re.sub(r",\s*'[XxYy]'\s*,", ', NULL,', content)
        
        # 7. Fix customer_types with NULL type_name
        if 'customer_types' in filename or 'part1' in filename:
            content = re.sub(
                r"(customer_types\s*\([^)]+\)\s*VALUES\s*\(\d+,\s*)NULL(\s*\))",
                r"\1'Unknown Type'\2",
                content
            )
...
def main():
    ...
    print("\n" + "="*60)
    print("🧹 MIGRATION DATA CLEANUP SCRIPT")
    print("="*60)
